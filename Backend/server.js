...
app.post('/api/tasks', verifyToken, async (req, res) => {
  try {
    const userId = safeObjectId(req.user.user_id);
    const { title, desc, status = 'Pending' } = req.body;
...
app.put('/api/tasks/:id', verifyToken, async (req, res) => {
  try {
    ...
    const { title, desc, status } = req.body; // ✅ include desc
    const updateData = {};
    if (title) updateData.title = title;
    if (desc !== undefined) updateData.desc = desc; // ✅ update desc
    ...
    res.json(updatedTask); // return full updated task
  } catch (err) {
